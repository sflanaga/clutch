        ...
        *self.ctrl.clone().keep_running.lock().unwrap() = true;
        ...
        self.handle = Some(thread::spawn(move || {
            ...
            while *c_ctrl.keep_running.lock().unwrap() {
                ...
                let res = c_ctrl.condstop.wait_timeout(lck, c_dur).unwrap();
                fun();
                lck = res.0;
                if res.1.timed_out() {
                    continue;
...
#[test]
fn test_it() {
    let mut timer = PeriodicThread::new();
    timer.start(|| println!("working.... "));
    ...
    timer.start(|| println!("working again..."));
    println!("main thread Feeling sleepy...");
    thread::sleep(time::Duration::from_millis(2000));
...
impl Stat {
    pub fn new(name: &str, verbosity: u32) -> Stat {
        Stat {
            name: name.to_string(),
...
impl StatTrack {
    ...
    pub fn add_stat(&mut self, name: &str, verbosity: u32) -> Arc<AtomicUsize> {
        let stat = Stat::new(name, verbosity);
